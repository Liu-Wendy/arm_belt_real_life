...
public class Continue extends BaseParameters {
    ...
    public ArrayList<ArrayList<Integer>> comb;
...
//				break;
    ...
    protected void Initialize() {
        ...
        for (int i = 0; i < SampleSize + PositiveNum; i++) {
            ...
            boolean exist = false;
            for (ArrayList<Integer> already : comb) {
                int k = 0;
                for (; k < auto_num; k++) {
                    if (already.get(k) != (int) temp[i].getFeature(k)) {
                        break;
                    }
                }
                if (k == auto_num) {
            ...
            if (!exist) {
                ...
                for (int l = 0; l < auto_num; l++) {
                    path.add((int) temp[i].getFeature(l));
                }
...
//		}
        ...
        if (!this.on_off) {
            ...
            for (int i = 1; i < this.MaxIteration; i++) {
                ...
                if ((ct - start_time) / 1000 > 3600) break;
...
//					break;
                ...
                if (bestValue < 0 && Math.abs(bestValue - preBestValue) < 0.00001) {
                    bestValueCount++;
                        ...
                        if (notExistInstance(j, NextPop[j])) {
                            ...
                            if (isGood) {
                                ...
                                for (ArrayList<Integer> already : comb) {
                                    int k = 0;
                                    for (; k < auto_num; k++) {
                                        if (already.get(k) != (int) NextPop[j].getFeature(k)) {
                                            break;
                                        }
                                    }
                                    if (k == auto_num) {
                                ...
                                if (!exist) {
                                    ...
                                    for (int l = 0; l < auto_num; l++) {
                                        path.add((int) NextPop[j].getFeature(l));
                                    }
...
//		of.valueArc.cover=eachBest.size();
        of.valueArc.cover = comb.size();
        of.valueArc.time_cost = new double[]{findFragTime, findPointTime, checkGoodOrNot, recordFragTime, stop_sum, sim, 0};
