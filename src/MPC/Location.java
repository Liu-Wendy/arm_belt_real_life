...
public class Location {
    ...
    public ArrayList<Transition> next_trans;
    ...
    public void setVariant(String variant,ArrayList<String> parameters){
        ...
        while(index != -1){
            ...
            variant = variant.substring(index+1).trim();
            index = variant.indexOf("&");
        }
        if(variant.length() > 0) processVariant(variant,parameters);
    }
...
//                String string = processMathFunction(strings[strings.length - 1]);
    ...
    public void setFlow(String flow,ArrayList<String> parameters){
        ...
        while(index != -1){
            ...
            flow = flow.substring(index + 2).trim();
            index = flow.indexOf("&&");
        }
        if(flow.length() > 0) processFlow(flow,parameters);
...
//        }
    ...
    public void clearFlow() {
        flows.entrySet().removeIf(entry -> entry.getKey().contains("arm"));
    }
}
