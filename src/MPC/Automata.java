...
public class Automata {
    ...
    public ArrayList<BindInfo> bindInfos;
    public ArrayList<String> parameters;
    public ArrayList<String> labels;
    public ArrayList<String> shared_labels;
    public int initLoc;
    ...
    public Automata(String aut_name){
        name=aut_name;
        ...
        shared_labels=new ArrayList<>();
        initParameterValues=new HashMap<>();
        parameters=new ArrayList<>();
        rangeParameters=new ArrayList<>();
...
//        boolean pruning = runRacos(automata, maxPathSize);
...
//    }
    ...
    boolean runRacos(Automata automata, int maxPathsize) {
//        int samplesize = 3;       // parameter: the number of samples in each iteration
//        int iteration = 2000;       // parameter: the number of iterations for batch racos
...
//        Instance ins = null;
//        int repeat = 1;
//       Task t = new ObjectFunction(automata,maxPathsize,PathMap);
...
//            System.out.println("total path choices: "+PathMap.size());
//            System.out.println("compute by flows costs:"+t.getTime()[0]);
//            System.out.println("compute costs:"+t.getTime()[3]);
...
//                double last=-1;
//                for(int k=0;k<maxPathsize;k++){
//                    if(ins.getFeature(k)==0) break;
//                    if(ins.getFeature(k)==last) continue;
//                    path_tmp.add((int)ins.getFeature(k));
//                    last=ins.getFeature(k);
//                }
//                minValueArc.path=new int[path_tmp.size()];
//                for(int k=0;k<path_tmp.size();k++){
//                    minValueArc.path[k]=path_tmp.get(k);
//                }
//            }
...
//            System.out.println("The init value of " + entry.getKey() + " is " + entry.getValue());
    ...
    public void createMap(int maxPathSize) {
        ArrayList<Integer> arrayListPath = new ArrayList<>();
        if (getInitLoc() != -1) {
            arrayListPath.add(getInitLoc());
    ...
    void DFS1(ArrayList<Integer> arrayListPath,int maxPathSize) {
        ...
        if (len == maxPathSize)
            return;
        ArrayList<Integer> neibours = locations.get(path[len - 1]).getNeibours();
        for (int i = 0; i < neibours.size(); ++i) {
            int nextPos = neibours.get(i);
            arrayListPath.add(neibours.get(i));
            DFS1(arrayListPath,maxPathSize);
...
//                   while(Math.pow(automata.target_x-automata.initParameterValues.get("x"),2) + Math.pow(automata.target_y-automata.initParameterValues.get("y"),2) > 2){
            ...
            automata.createMap(maxPathSize);
...
//                   while(Math.abs(200 - automata.initParameterValues.get("x")) + Math.abs(200-automata.initParameterValues.get("y")) > 2){
                automata.runRacos(automata,maxPathSize);
//                ArrayList<Integer> arrayListPath = new ArrayList<>();
